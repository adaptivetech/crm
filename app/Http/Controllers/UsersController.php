...
class UsersController extends Controller
{
     ...
     * @return mixed
     */
    public function debtData($id)
    {
        $debts = Debt::select(
            ['id', 'title', 'created_at', 'user_assigned_id', 'client_id', 'status']
        )
            ->where('user_assigned_id', $id);
        return Datatables::of($debts)
            ->addColumn('titlelink', function ($debts) {
                return '<a href="' . route('debts.show', $debts->id) . '">' . $debts->title . '</a>';
            })
            ->editColumn('created_at', function ($debts) {
                return $debts->created_at ? with(new Carbon($debts->created_at))
                    ->format('d/m/Y') : '';
            })
            ->editColumn('status', function ($debts) {
                return $debts->status == 1 ? '<span class="label label-success">Open</span>' : '<span class="label label-danger">Closed</span>';
            })
            ->editColumn('client_id', function ($tasks) {
                return $tasks->client->name;
            })
    ...
    public function show($id)
    {
        return view('users.show')
            ...
            ->withDebtStatistics($this->debts->totalOpenAndClosedDebts($id));
    }
